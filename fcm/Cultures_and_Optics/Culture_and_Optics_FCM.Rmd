...
Cell abundances for SYN22-5 are low quality due to instrumentation saturation:
- concentrations in the optics system will be corrected using the relationship between cell abundance and chlorophyll (from ACS) later
- the average ratio of cells in the optics system relative to the cultures from SYN22-4 will be used to estimate the SYN22-5 culture concentrations
...
```{r lmodel2 of cells and chl for syn22-4 optics, message=FALSE, wrapper=TRUE}
...
```{r calculate ratio of optics cells v culture cells for syn22-4, message=FALSE, wrapper=TRUE}
...
```{r estimate syn22-5 optics cell counts using syn22-4 lmodel2, message=FALSE, wrapper=TRUE}
...
Here we subtract the cell concentration in the POC filtrates from the culture cell abundance to estimate the abundance on the POC filters
...
Even when cells slip through and are detected by the flow cytometer, 98.9 +/- 0.9% cells are captured on the filter
