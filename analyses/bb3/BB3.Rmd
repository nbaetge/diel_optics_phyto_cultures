...
```{r wrangle bb data and combine, message=FALSE, warning=FALSE, wrapper=TRUE}
...
```{r count plot, fig.height=10, fig.width=16, message=FALSE, warning=FALSE, wrapper=TRUE}
data %>% 
ggplot(aes(
    x = wl,
    y = med,
...
```{r summarize and merge with fcm data, message=FALSE, wrapper=TRUE}
bb3_sum <- bbp %>%
