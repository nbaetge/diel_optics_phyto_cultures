...
```{r wrangle acs data and combine, message=FALSE, warning=FALSE, wrapper=TRUE}
...
```{r filtrate attenuation plot, fig.height=10, fig.width=16, message=FALSE, warning=FALSE, wrapper=TRUE}
c_filt %>%
  ggplot(aes(
    x = wl,
    y = med,
...
```{r filtrate absorption plot, fig.height=10, fig.width=16, message=FALSE, warning=FALSE, wrapper=TRUE}
a_filt %>%
  ggplot(aes(
    x = wl,
    y = med,
...
This is done using the MATLAB script "rd_data.m" written by Emmanuel Boss
-files needed are ResidualTemperatureAndScatteringCorrection.m and Sullivan_etal_2006_instrumentspecific.xls

-

```{r export data for correction, wrapper=TRUE}
...
```{r import and wrangle corrected data, message=FALSE, wrapper=TRUE}
corrected_files <-
...
```{r correct spectra to remove imprint of previous sample, wrapper=TRUE}
...
```{r corrected ap plot, wrapper=TRUE}
...
```{r corrected cp plot, wrapper=TRUE}
...
```{r pariculate coefficient plots, fig.asp=0.8, fig.width=24, message=FALSE, warning=FALSE, wrapper=TRUE}
...
calculating chlorophyll concentrations based on Roesler and Barnard (2015), where chl (mg/m^3) = (alh (m^-1) + 0.012 (m^1)) / 0.0104 (m^2/mg)

```{r calculate chl, message=FALSE, wrapper=TRUE}
...
```{r interpolate values for bb derivations, message=FALSE, warning=FALSE, wrapper=TRUE}
...
```{r summarize fcm data, message=FALSE, wrapper=TRUE}
acs_sum <- acs_bottle %>%
